...
def test_router_init_ipv4_fritzbox(mocker):
    mocker.patch('fritzconnection.FritzConnection', return_value=Mock())
    mocker.patch(
...
def test_updater_write_read_cache(mocker, tmp_path):
    ...
    assert updater._cache == test_cache_content
    assert (
        updater.get_cache_domain(testdata_domain_name)
        == test_cache_content[testdata_domain_name]
    )
    ...
    updater.update_cache_domain(
        testdata_domain_name, test_cache_content[testdata_domain_name]
    )
    updater.update_cache_domain(
        testdata_domain_name_other, test_cache_content[testdata_domain_name_other]
    )
    assert (
        updater.get_cache_domain(testdata_domain_name)
        == test_cache_content[testdata_domain_name]
    ...
    assert (
        updater.get_cache_domain(testdata_domain_name_other)
